...
int str_cmp_v(cadena * cad_a,cadena * cad_b){
	...
	while((i<nodos_a) || (j<nodos_b)){
		...
		//El array "a" ha llegado al final de la estructura "i"
		if(ii == a[i].iov_len){ 
			i++;
			ii=0;
		}

		//El array "b" ha llegado al final de la estructura "j"
		if(jj == b[j].iov_len){
			j++;
			jj=0;
		}
		...
		//Comprueba final
		if((i==nodos_a) || (j==nodos_b)) break;
		
		//Compara "a" y "b" caracter a caracter
		if (strncmp(a[i].iov_base+ii,b[j].iov_base+jj,1)) return 0;
